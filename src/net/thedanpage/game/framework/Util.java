...
public class Util {
	...
	/**
	 * Used to round decimals when the coordinates of the player are displayed on screen.
	 */
	...
	/**
	 * Get a list of files from the resources directory in the format of a string array.
	 * Solution taken from <a href="https://stackoverflow.com/questions/28985379/java-how-to-read-folder-and-list-files-in-that-folder-in-jar-environment-instead">Stack Overflow</a>
	 * @param folderpath
	 * @return
	 */
	...
	/**
	 * Format a double to a string (format: #.####) because it looks nice with 4-decimal rounding.
	 * 3 cases are included because Java is stupid and has inconsistent double formatting. <333
	 * This is used for formatting the player's coordinates when they're shown on the screen.
	 * {@link #dfForString} is used in order to do the formatting.
	 * @param d A double to be formatted
	 * @return
	 */
	public static String formatDoubleForString(double d) {
		if (d == 0) return "0.0000";
		if (d > 0 && d < 1) return "0" + dfForString.format(d);
