...
public class AnimatedTexture extends Texture {
	...
	/**
	 * An array containing pixel data for each frame. The first dimension specifies the
	 * frame number, while the second dimension contains the actual RGB data.
	 */
	 ...
	 * @param images	a string array consisting of image paths, relative to the resources/images folder
	...
	/*
	 *  This is an animated texture, you can't get data without specifying a frame! Lol
	 */
	@Override
	public int getWidth() {
		return 0;
	...
	public int getWidth(int frame) {
		if (frame >= 0 && frame < this.frameWidth.length) return frameWidth[frame];
		return 0;
	}
	...
	public int getHeight(int frame) {
		if (frame >= 0 && frame < this.frameHeight.length) return frameHeight[frame];
		return 0;
	}
	...
	public int[] getPixels(int frame) {
		if (frame >= 0 && frame < this.frames.length) return frames[frame];
		return null;
	}
